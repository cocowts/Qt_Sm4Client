...
unsigned int sm4_PKCS7Padding(unsigned char* buf, unsigned int length)
{
    ...
    for(i=length; i<length+padding_count; i++)
        buf[i] = padding_count;

    return (length + padding_count);
}

/*
 * SM4 PKCS7Restore
 */
unsigned int sm4_PKCS7Restore(unsigned char* buf, unsigned int length)
{
    unsigned char padding_count = buf[length - 1];
    unsigned char count = 0;
    unsigned int i = 0;

    for(i=length-padding_count; i<length; i++)
    {
        if(buf[i] != padding_count)
        {
            break;
        }
        else
